...
export default function Product() {
    ...
    const {cart, setCart, setNotification, setOpenNotificationBox} = useContext(DataContext);
    ...
    function handleAddCart(e) {
        ...
        ...prevCart,
        {
            ...
            total: (Number(piece) * parseFloat(data.price))
        }
        ]);
    ...
    const [data, setData] = useState(products.filter(x => (x.id === params.id))[0])
        ...
        <div className="product-page">
            ...
            <div className="product-page-top">
                ...
                <div className="product-page-image-box">
                    <button onClick={() => (setPhotoOrder(1))} style={{left: '0'}}><i className="fa-solid fa-chevron-left"></i></button>
                    <img src={photoOrder === 1 ? data?.image01 : data?.image02} alt="" />
                    <button onClick={() => (setPhotoOrder(2))} style={{right: '0'}}><i className="fa-solid fa-chevron-right"></i></button>
                </div>
                ...
                <div className="product-content">
                    ...
                    <p>Lorem, ipsum dolor sit amet consectetur adipisicing elit. Quod molestias laudantium, optio incidunt quis nobis qui dolorum consequuntur animi necessitatibus nam unde ab commodi voluptate possimus error maxime harum praesentium!</p>

                    <div className="select">
                        <p>Beden Seç</p>
                        <div>
                            <button style={size === 'xxs' ? {backgroundColor: '#e2e8f0'} : {}} onClick={() => (setSize('xxs'))}>XXS</button>
                            <button style={size === 'xs' ? {backgroundColor: '#e2e8f0'} : {}} onClick={() => (setSize('xs'))}>XS</button>
                            <button style={size === 's' ? {backgroundColor: '#e2e8f0'} : {}} onClick={() => (setSize('s'))}>S</button>
                            <button style={size === 'm' ? {backgroundColor: '#e2e8f0'} : {}} onClick={() => (setSize('m'))}>M</button>
                            <button style={size === 'l' ? {backgroundColor: '#e2e8f0'} : {}} onClick={() => (setSize('l'))}>L</button>
                        </div>
                    </div>
                    ...
                    <div className="select">
                        <p>Adet Seç</p>
                        <span>
                            <button onClick={() => (piece > 1 && setPiece(piece - 1))}><i className="fa-solid fa-angle-left"></i></button>
                            <p>{piece}</p>
                            <button onClick={() => (setPiece(piece + 1))}><i className="fa-solid fa-angle-right"></i></button>
                        </span>
                    </div>
            ...
            <div className="product-page-bottom">
                ...
                <main>
                    {
                        products?.filter(x => (x.gender === params?.category)).slice(0,4).map(x => (
                            <Link to={`/urun/${x.id}`} key={x.id} className="product-item">
